...
public class Prober3 {
	...
	public Prober3(int id, int nnodes, Network3 network, NodeRunner3 nodeRunner, FailureDetector failureDetector) {
		...
		if(mynode == 0) {
			ProbeMessage3 token = new ProbeMessage3(mynode, nnodes, nodeRunner.getState().getLc());
			token.incSeq();
			PerformanceLogger.instance().addTokenBits(3, token.copy());
			sendProbeMessage(token, 0);
		}
	}
	...
	private void handleToken(ProbeMessage3 token) {
		...
		sendProbeMessage(token, nodeRunner.getNext());
	...
	public void sendProbeMessage(ProbeMessage3 probeMessage, int destination) {
		// update messages lamport clock value
		probeMessage.setLc(nodeRunner.getState().getLc());
		network.sendProbeMessage(probeMessage, destination);
